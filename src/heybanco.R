...
# Crear el nombre de la ruta del archivo CSV para el mes y año seleccionados
var_name <- paste0("data/mensual/mes", mes, "_", ano, "/mes", mes, "_", ano, ".csv")

# Verificar si el archivo existe
if (!file.exists(var_name)) {
  stop("No se encontró el archivo para el mes y año seleccionados.")
}

# Obtener la lista de archivos
archivos <- get(var_name)

# Leer los archivos y combinarlos en un solo vector
texto_vector <- unlist(lapply(archivos, function(x) {
  # Leer el archivo
...
texto_vector <- gsub("[[:punct:]]", "", texto_vector)

# Verificar los resultados
unique(texto_vector) %>% sort()
